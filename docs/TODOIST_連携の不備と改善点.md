...
### 1. **ログイン解除とAPIの不整合（重要）**
- **現象**: ログイン必須を解除したため、`getServerSession()` は常に未ログインになる。
- **影響**:
  - **POST /api/todoist/create-task** … チェックリストから Todoist にタスクを作成する際に **常に 401** となり、本番では「Todoist にタスクを作成」が動かない。
  - **GET/POST /api/todoist/user-token** … 同様に 401 になる（現状はトークンをローカルストレージに保存しているため影響は小さい）。
- **推奨対応**: 未ログイン時は「リクエストボディの `userToken` があれば認証とみなす」など、トークンがあれば create-task を通すようにする。
...
### 3. **sync-status API がユーザートークンに対応していない**
- **現象**: `POST /api/todoist/sync-status` は `hasToken()`（サーバーの `TODOIST_API_TOKEN`）のみを見ている。リクエストで `userToken` を送れない。
- **影響**: 本番でサーバーに `TODOIST_API_TOKEN` を設定していない場合、「進捗を同期」ボタンが **401 で失敗**する。
- **推奨対応**: リクエストボディで `userToken` を受け取り、そのトークンで Todoist API を叩くようにする。
...
| 項目 | 深刻度 | 状態 |
|------|--------|------|
| ログイン解除後の create-task が 401 | 高 | 要修正 |
| sync-status が userToken 非対応 | 高 | 要修正 |
| Webhook 受信後の進捗更新 | 中 | 未実装（DB 等が必要） |
| 担当者を Todoist に反映 | 中 | 未実装 |
| 優先度の連携 | 低 | 未実装 |
| Webhook シークレット未設定時の挙動 | 低 | 要検討 |
| データ永続化 | 設計 | DB 導入で検討 |

まずは **1 と 3**（create-task の認証緩和と sync-status の userToken 対応）を直すと、ログインなし・サーバーにトークンなしでも「タスク作成」と「進捗を同期」が本番で動くようになります。
